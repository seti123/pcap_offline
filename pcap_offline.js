// PCAP offline library 
// based on mranny/node_pcap and pcap-stream, MIT Licence
...
Pcap.prototype.readPcapStream = function (fileName, packetHandler) 
{
    pstream = new PcapStream(fileName);
    console.log ('new stream');
    pstream.on('readable', function (){
		  var msg = pstream.read();
		  if (msg) 
		  {
			// MAKE PCAP-PARSER header compatiple with PCAP
			//console.log (tc++);
			//var header = 
			msg.data.pcap_header={
				link_type: 'LINKTYPE_ETHERNET', 
				time_ms: msg.pcap.timestampSeconds*1000 , 
				len: msg.pcap.originalLength,
				caplen: msg.pcap.capturedLength};; 
			//debug: console.log ("NEW PCAP HEAEDER:" + JSON.stringify (msg.data.pcap_header));
			packetHandler.call (this, msg.data);
		  }
...
exports.readPcap = function (fname, callback) {
	var session = new Pcap();
	session.readStream = session.readPcapStream (fname, callback);
	return session;
}
...
decode.packet = function (raw_packet) {
    ...
    //for (var prop in packet.pcap_header)
    //	raw_packet.pcap_header[prop]=packet.pcap_header[prop];
    	

    return packet;
};

findIpv4Offset = function (raw_packet, offset) {
    var ret = {};
    var original_offset = offset;
    offset += 4;
    ret.payloadLength = unpack.uint16(raw_packet, offset); 
    //console.log ("PPP paylod length = " + ret.payloadLength);
    // search for IP v4
    for (var i=offset+2; i<offset+ret.payloadLength; i++)
    {
        if (raw_packet[i]==0x45)
        {
            //console.log ('found ip4 header at ' + (i-offset));
            offset=i;
            break;
        }
    }
    //offset += 2;
    //offset = offset + ret.payloadLength;
...
decode.ethernet = function (raw_packet, offset) {
    ...
    if (ret.ethertype < 1536) {
        ...
        case 0x88cc: // LLDP - http://en.wikipedia.org/wiki/Link_Layer_Discovery_Protocol
            ...
            break;
        case 0x8864: // PPoE - http://en.wikipedia.org/wiki/Link_Layer_Discovery_Protocol
            //ret.pppop = decode.pppoe(raw_packet,offset);
            ret.ip = decode.ip(raw_packet, findIpv4Offset (raw_packet,offset));
        default:
        	console.log("pcap.js: decode.ethernet() - Don't know how to decode ethertype " + ret.ethertype + ' try findIpv4Packets ...'); 
            var v4offset = findIpv4Offset (raw_packet,offset)
            ret.ip = decode.ip(raw_packet, v4offset);
            console.log("found ipV4 packets at offset: " + v4offset);
        }
    }
...
decode.udp = function (raw_packet, offset) {
    ...
    if (ret.data_bytes > 0) 
    {
    	// todo fails for 48 packets in testfile -> dump packet to error file
        ret.data = raw_packet.slice(ret.data_offset, ret.data_end); //attention add -1 TODO TEST or check buffer length, fragmented IP?
...
decode.dns = function (raw_packet, offset) {
    ...
    for (i = 0; i < ret.header.qdcount ; i += 1) {
        ...
        question_done = false;
        parts = [];
        //console.log ("DECODE DNS1:" + JSON.stringify(raw_packet.pcap_header));
        ...
        if (tcp.data_bytes) {
            ...
            if (session.send_packets[tcp.seqno + tcp.data_bytes]) {
                ...
                if (session.http_detect) {
                	...
                	this.emit('data', session,  {direction: 'send' , data: tcp.data});
                }
            }
            ...
            if (session.recv_packets[tcp.seqno + tcp.data_bytes]) {
                ...
                if (session.http_detect) {
                    ...
                    // TODO - check for WS parser errors
                	 ...
                	 this.emit('data', session,  {direction: 'recv' , data: tcp.data});
                }
            }
